...
export default function QuetPhieuLienTuc() {
  ...
  const chupVaQuet = () => {
    ...
    const maxDim = 1600; // Nâng lên 1600px để đảm bảo các chữ số cực nhỏ đều nét căng
    ...
    if (ctx) {
      ...
      // Nén ảnh chất lượng 0.95: Giữ nguyên độ sắc nét tối đa cho OCR chính xác 100% nhưng tối ưu được dung lượng truyền
      const base64 = c.toDataURL('image/jpeg', 0.95);
      guiQuetAI(base64);
    }
  };
  ...
  const chonFile = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      img.onload = () => {
        ...
        const maxDim = 2000; // Đảm bảo độ phân giải cực cao (2000px) cho các tài liệu có chữ viết tay nhỏ
        ...
        // Nén ảnh chất lượng 0.95 để giữ độ nét cao nhất cho các chi tiết chữ viết tay nhỏ
        const compressedB64 = tempCanvas.toDataURL('image/jpeg', 0.95);
